...
import "./SponsoredTxInfo.sol";
import "hardhat/console.sol";

contract ActionTokenPaymaster is ITokenPaymaster, Ownable {
    ...
    function setActionToken(
        address[] calldata _actionTokens
    ) external onlyOwner {
        for (uint256 i = 0; i < _actionTokens.length; i++) {
            actionToken[_actionTokens[i]] = true;
        }
    }
    ...
    function _validatePaymasterUserOp(
        ...
        require(
            ...
            "ActionTokenPaymaster: gas too low for postOp"
        );

        console.logBytes(userOp.paymasterAndData);
        console.log(userOp.paymasterAndData.length);

        // paymasterAndData: [paymaster, token, maxCost], 20 + 20 + 32 bytes = 72
        // bytes calldata paymasterAndData = userOp.paymasterAndData;
        // require(
        //     userOp.paymasterAndData.length == 72,
        //     "ActionTokenPaymaster: paymasterAndData must specify token"
        // );
        console.logBytes(userOp.paymasterAndData[20:]);
        ...
        if (actionToken[token]) {
            ...
            // action tokens do not require gas
            require(
                ERC20Token.allowance(sender, address(this)) >= 1,
                "ActionTokenPaymaster: not enough allowance"
            );
            console.log("passed allowance");
            // abi encode: sender, action token address, 1 action token, exchange rate is 0
            return (abi.encode(sender, token, 1, 0), 0);
        } else {
            ...
            uint256 exchangeRate = (_price * 10 ** tokenDecimals) /
                10 ** _decimals; // ./10^18
            ...
            require(
                tokenRequiredPreFund <= maxCost,
                "ActionTokenPaymaster: maxCost too low"
            );
            ...
            require(
                ...
                "ActionTokenPaymaster: not enough balance"
            );

            return (abi.encode(sender, token, costOfPost, exchangeRate), 0);
        }

        // TODO: custome sponsored tx
        // address sender = userOp.getSender();
        // paymasterAndData: [paymaster, sponsoredTxInfo]
        // SponsoredTxInfo memory sponsoredTxInfo = abi.decode(
        //     userOp.paymasterAndData[20:],
        //     (SponsoredTxInfo)
        // );

        // if (sponsoredTxInfo.jediMindTrickSignature.length > 0) {
        //     if (userOp.initCode.length != 0) {
        //         _validateConstructor(
        //             userOp,
        //             sponsoredTxInfo.token,
        //             requiredPreFund
        //         );
        //     }
        ...
        //     // TODO: check authorized method with signature

        //     return ("", 0);
        // } else {
        //     IERC20 ERC20Token = IERC20(sponsoredTxInfo.token);

        //     (uint256 _price, uint8 _decimals) = this.exchangePrice(
        //         sponsoredTxInfo.token
        //     );
        //     uint8 tokenDecimals = IERC20Metadata(sponsoredTxInfo.token)
        //         .decimals();
        ...
        //     require(
        //         tokenRequiredPreFund <= sponsoredTxInfo.maxCost,
        //         "Paymaster: maxCost too low"
        //     );
    ...
    function _postOp(
        ...
        if (actionToken[token]) {
            IERC20(token).safeTransferFrom(sender, address(this), 1);
        } else {
